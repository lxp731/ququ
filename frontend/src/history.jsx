      ...
      <div className="px-6 py-4 flex-shrink-0">
        <div className="relative max-w-2xl mx-auto">
          ...
          <input
            ...
            placeholder="搜索转录内容..."
            className="w-full pl-10 pr-4 py-2.5 text-sm bg-white/4 border border-white/8 rounded-xl focus:ring-2 focus:ring-indigo-500/50 focus:border-transparent text-white placeholder-white/20 transition-all"
          />
        </div>
      </div>
            ...
            <div className="space-y-2">
              <AnimatePresence>
                {filtered.map((item, idx) => (                  <motion.div
                    ...
                    className={`glass-light p-4 cursor-pointer transition-all hover:border-white/20 ${selected?.id === item.id ? 'border-indigo-400/30 bg-indigo-500/4' : ''}`}
