...
import { Settings, Save, Loader2, TestTube, Mic, Shield, Zap, Cpu, ChevronDown, Server } from 'lucide-react';
import './index.css';
...
const Input = ({ label, hint, ...props }) => (
  <div>
    ...
    <input {...props} className="w-full px-3 py-2 text-sm bg-white/4 border border-white/8 rounded-lg focus:ring-2 focus:ring-indigo-500/50 focus:border-transparent text-white placeholder-white/20 transition-all" />
  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => { setOpen(!open); setCustomizing(false); }}
        className="w-full flex items-center justify-between px-3 py-2 text-sm bg-white/4 border border-white/8 rounded-lg hover:border-white/20 transition-all text-white text-left">
      ...
      {open && (
        <div className="absolute z-50 top-full left-0 right-0 mt-1 max-h-56 overflow-y-auto rounded-lg border border-white/8 bg-[#1a1a2e] backdrop-blur-xl shadow-2xl">
          {customizing ? (
            <div className="flex items-center gap-2 px-2 py-2 border-b border-white/6">
              <input ref={inputRef} autoFocus type="text" value={draft}
                ...
                placeholder="输入模型名称…"
                className="flex-1 px-2 py-1.5 text-sm bg-white/6 border border-white/8 rounded text-white placeholder-white/20 focus:outline-none focus:border-indigo-500/50" />
            ...
            <button type="button" onClick={() => { setCustomizing(true); setDraft(value && !isPreset ? value : ''); }}
              className="w-full text-left px-3 py-2 text-sm text-white/40 hover:bg-white/8 transition-colors border-b border-white/6">
              ⚡ 自定义模型…
            </button>
          )}
          {fetching && (
          ...
          {options.length > 0 ? (
            options.map(o => (
              <button key={o.value} type="button" onClick={() => handleSelect(o.value)}
                className={`w-full text-left px-3 py-2 text-sm transition-colors hover:bg-white/8 ${o.value === value ? 'text-indigo-300 bg-indigo-500/10' : 'text-white/70'}`}>
                {o.label}
              </button>
            ))
...
const SettingsPage = () => {
  const [settings, setSettings] = useState({
    ...
    hotword_path: '', hotwords: '', hotwords_count: 0,
    funasr_base_url: '',
  });
  ...
  useEffect(() => {
    const unsub = window.electronAPI?.onSettingsUpdate((data) => {
      if (data?.hotwords_count !== undefined) {
        setSettings(p => ({ ...p, hotwords_count: data.hotwords_count }));
      }
    });
    return () => unsub?.();
  }, []);
    ...
    try {
      ...
      if (window.electronAPI) {
        ...
        setSettings({
          ...
          hotword_path: all.hotword_path || '',
          hotwords: all.hotwords || '',
          hotwords_count: all.hotwords_count || (all.hotwords || '').split(',').filter(Boolean).length,
          funasr_base_url: all.funasr_base_url || '',
        });
      }
    } catch (_e) { toast.error('加载设置失败'); }
  ...
  const testConfig = async () => {
    ...
    } catch (_e) { toast.error('测试失败', { description: _e.message }); }
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        <div className="max-w-xl mx-auto p-6 space-y-6 pb-10">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            ...
            <h2 className="text-sm font-semibold text-white/80 mb-3">常用词</h2>
            <p className="text-xs text-white/40 mb-3">
              选择 txt 文件（一行一词），提升特定词汇识别准确率。
            </p>
            <div className="flex items-center gap-3">
              <button
                onClick={async () => {
                  if (!window.electronAPI) return;
                  const r = await window.electronAPI.selectHotwordFile();
                  if (r?.canceled) return;
                  if (r?.error) { toast.error(`读取失败: ${r.error}`); return; }
                  setSettings(p => ({ ...p, hotword_path: r.path, hotwords: r.words.join(','), hotwords_count: r.count }));
                  toast.success(`已加载 ${r.count} 个常用词`);
                  // 同步到后端 + 持久化
                  try {
                    await window.electronAPI.saveSetting('hotword_path', r.path);
                    await window.electronAPI.saveSetting('hotwords', r.words.join(','));
                  } catch (_) { /* ignore */ }
                }}
                className="px-3 py-1.5 text-xs rounded-lg bg-indigo-500/20 text-indigo-300 hover:bg-indigo-500/30 transition-colors"
              >
                选择热词文件
              </button>
              {settings.hotword_path && (
                <span className="text-xs text-white/50 truncate max-w-[200px]" title={settings.hotword_path}>
                  {settings.hotword_path.split('/').pop()} ({settings.hotwords_count || (settings.hotwords || '').split(',').filter(Boolean).length} 词)
                </span>
              )}
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            ...
            <div className="p-4 rounded-lg bg-indigo-500/4 border border-indigo-500/10">
